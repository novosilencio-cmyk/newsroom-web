(function () {
  const exercises = [
    ...
    ['Trace each claim', 'Underline five important claims. For each, identify its document, named source or observation, or mark it as your own analysis and state the reasoning.', 'Five claims, each paired with its evidence or reasoning.', 'Could a reader mistake an interpretation for a documented fact?'],
    ...
    ['Spot a persuasive overstatement', 'Write a fluent sentence that deliberately goes beyond your sources. Label it as a rejected practice example and mark the word or phrase that makes the claim too strong.', 'The rejected sentence and one line explaining what its source actually supports.', 'Does your real draft make a subtler version of the same unsupported claim?'],
    ...
    ['Invite constructive disagreement', 'State exactly what your conclusion covers, then name the evidence that would make you revise it.', 'A conclusion and the evidence that would change your assessment.', 'Could a good-faith critic tell what to test next?'],
  ...
  document.getElementById('daily-intro').textContent = 'Practice ' + (index + 1) + ' of 14 · one focused exercise for your next factual draft.';
